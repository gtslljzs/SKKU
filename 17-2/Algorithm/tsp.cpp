...
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
    ...
    *cost = 0;
    ...
    fill( coveredColumns, coveredColumns + nOfColumns, false );
    coveredRows = new bool [nOfRows];
    fill( coveredRows, coveredRows + nOfRows, false );
    starMatrix = new bool [nOfElements];
    fill( starMatrix, starMatrix + nOfElements, false );
    primeMatrix = new bool [nOfElements];
    fill( primeMatrix, primeMatrix + nOfElements, false );
    newStarMatrix = new bool [nOfElements];
    fill( newStarMatrix, newStarMatrix + nOfElements, false );
    ...
    for ( int i = 0; i < nOfRows; i++ ) {
        for ( int j = 0; j < nOfColumns; j++ ) {
            if ( i == j )
                ...
                printf( "%.2f ", distMatrix[j + i * nOfColumns] );
        }
        printf( "\n" );
    }
    printf("\n");
    ...
    for ( int i = 0; i < nOfRows; i++ ) {
        for ( int j = 0; j < nOfColumns; j++ )
            printf( "%d ", starMatrix[j + i * nOfColumns] );
        printf( "\n" );
    }
    printf("\n");
...
int main( int argc, char** argv )
{
    ...
    fscanf( ifp, "%d", &p_num );
    points = new cor [p_num];
    
    for ( int i = 0; i < p_num; i++ ) {
        fscanf( ifp, "%lf %lf", &points[i].x, &points[i].y );
        points[i].idx = i;
    }
    
    fclose( ifp );
    
    vector< vector<double> > costMatrix (p_num);
    for ( int i = 0; i < p_num; i++ ) {
        for ( int j = 0; j < p_num; j++ ) {
            if ( i == j )
                costMatrix[i].push_back( numeric_limits<double>::max() );
            else
                costMatrix[i].push_back( Dist( points[i], points[j] ) );
        }
    }
